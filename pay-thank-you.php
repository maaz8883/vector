...
$lead_id = $linkData ? htmlspecialchars($linkData['lead_uuid'] ?? $uuid, ENT_QUOTES, 'UTF-8') : '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>Order Confirmed — <?php echo htmlspecialchars($fun_site); ?></title>
...
.scroll-arrow:hover { animation-play-state: paused; transform: scale(1.1); }
...
.addons-page { max-width: 920px; margin: 0 auto; padding: 0 1.25rem 2.5rem; }
.page-header { text-align: center; margin-bottom: 2rem; }
.promo-badge {
  display: inline-flex; align-items: center; gap: 6px;
  background: var(--warn-bg); color: var(--warn-text);
  font-size: 12px; font-weight: 600; padding: 5px 14px;
  border-radius: 30px; margin-bottom: 12px; letter-spacing: 0.3px;
}
.page-header h2 { font-size: 30px; font-weight: 700; color: #1a1a1a; margin-bottom: 6px; }
...
.addon-body h3 { font-size: 14px; font-weight: 600; color: #1a1a1a; margin-bottom: 4px; line-height: 1.3; }
...
.bar-left small { display: block; font-size: 12px; color: #999; margin-bottom: 4px; font-weight: 500; letter-spacing: 0.3px; }
.bar-left strong { font-size: 28px; font-weight: 700; color: #1a1a1a; line-height: 1; }
...
.site-footer { background: #111; padding: 0; margin-bottom: 0; }
.footer-copy-bar { background: var(--brand); padding: 12px 40px; text-align: center; }
.footer-copy-bar p { font-size: 12.5px; color: rgba(255,255,255,0.88); letter-spacing: 0.2px; }
.footer-copy-bar p a { color: #fff; font-weight: 600; text-decoration: none; }
...
@media (max-width: 600px) {
  ...
  .order-confirm-card { padding: 1rem 0.85rem !important; margin: 15px auto 10px !important; }
  .order-confirm-header { flex-direction: column !important; text-align: center !important; gap: 0.75rem !important; margin-bottom: 1rem !important; }
  .order-confirm-header > div:first-child { width: 50px !important; height: 50px !important; }
  .order-confirm-header > div:first-child i { font-size: 20px !important; }
  .order-confirm-header h1 { font-size: 20px !important; margin-bottom: 4px !important; }
  .order-confirm-header p { font-size: 12px !important; line-height: 1.4 !important; }
  .order-details-grid { grid-template-columns: 1fr !important; gap: 0 !important; padding: 0.75rem !important; }
  .order-details-grid > div { border: none !important; padding: 0.5rem 0 !important; border-bottom: 1px solid #e0e0e0 !important; text-align: left !important; }
  .order-details-grid > div:last-child { border-bottom: none !important; padding-bottom: 0 !important; }
  .order-details-grid > div > div:first-child { font-size: 10px !important; margin-bottom: 3px !important; }
  .order-details-grid > div > div:last-child { font-size: 14px !important; }
}
</style>
  ...
  <div class="order-confirm-card" style="background: #fff; border-radius: 20px; padding: 1.75rem 2rem; box-shadow: 0 2px 12px rgba(0,0,0,0.06); border: 1px solid #ebe9e4;">
    <div class="order-confirm-header" style="display: flex; align-items: center; gap: 1.5rem; margin-bottom: 1.5rem;">
      ...
      <div style="flex: 1;">
        ...
        <p style="font-size: 14px; color: #6b6b6b; margin: 0; line-height: 1.5;">Thank you for choosing <?php echo htmlspecialchars($fun_site); ?>. Our team will begin working on your project immediately.</p>
      </div>
    </div>
...
<footer class="site-footer">
  <div class="footer-copy-bar">
    <p>&copy; <?php echo date('Y'); ?> <a href="#"><?php echo htmlspecialchars($fun_site); ?></a>. All rights reserved.</p>
  </div>
</footer>
...
<script>
<?php if ($linkData): ?>
const LEAD_ID = <?= json_encode($linkData['lead_uuid'] ?? $uuid) ?>;
